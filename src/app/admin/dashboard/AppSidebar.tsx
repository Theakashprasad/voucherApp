...
export function DashboardSidebar() {
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 border-r border-gray-200/50"
    >
      <SidebarHeader className=" bg-white/80 backdrop-blur-sm p-4">
          ...
          {items.map((item) => (
            <SidebarMenuItem key={item.label}>
              <SidebarMenuButton
                ...
                className={`transition-all h-13 duration-200 rounded-xl hover:shadow-md ${
                  pathname === item.href
                    ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg"
                    : "bg-white/60 backdrop-blur-sm text-gray-700 hover:bg-white/80 hover:scale-105"
                }`}
              >
                <Link
                  href={item.href}
      ...
      <SidebarFooter className="border-t border-gray-200/50 bg-white/80 backdrop-blur-sm p-4">
        <Toaster position="top-right" expand={true} />

        <Dialog>
          <DialogTrigger asChild>
            <SidebarMenuButton asChild tooltip="Profile" className="h-auto">
              <button
                type="button"
                aria-label="Profile"
                className={`flex items-center transition-all duration-200 rounded-xl hover:shadow-md ${
                  state === "collapsed"
                    ? "justify-center w-full p-3 bg-white/60 backdrop-blur-sm hover:bg-white/80 hover:scale-105"
                    : "gap-3 w-full text-left bg-white/60 backdrop-blur-sm hover:bg-white/80 hover:scale-105 rounded-xl p-3"
                }`}
              >
                {state === "collapsed" ? (
                  <Icons.Contact className="text-gray-700 h-5 w-5" />
                ) : (
                  <Image
                    ...
                    className="h-8 w-8 flex-shrink-0 rounded-full border-2 border-gray-200 object-cover"
                    width={32}
                    height={32}
                    alt="Avatar"
                  />
                ...
                {state !== "collapsed" && (
                  <div className="flex flex-col">
                    ...
                    <span className="font-sans text-xs text-gray-500 truncate max-w-[120px]">
